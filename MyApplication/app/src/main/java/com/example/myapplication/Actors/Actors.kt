...
@Composable
fun ScreenProfil(windowClass: WindowSizeClass,homeViewModel: HomeViewModel) {
    var text by remember { mutableStateOf("") }
    var active by remember { mutableStateOf(false) }
    val actors by homeViewModel.actors.collectAsState()
    Scaffold {
        SearchBar(query = text, onQueryChange = { text = it },
            modifier = Modifier.fillMaxWidth(),
            onSearch = {
                if (text=="") {
                    homeViewModel.getActors()
                }
                active = false
                homeViewModel.searchActors(text)
            },
            active = active,
            onActiveChange = {
                active = it
